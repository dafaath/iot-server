...
func (u *HardwareRepository) GetHardwareTypeById(ctx context.Context, tx helper.Querier, id int) (hardwareType string, err error) {
	sqlStatement := `SELECT type FROM "hardware" WHERE id_hardware=$1`
	err = tx.QueryRow(ctx, sqlStatement, id).Scan(&hardwareType)
	if err != nil {
		if err == pgx.ErrNoRows {
			return "", fiber.NewError(404, fmt.Sprintf("Hardware with id %d not found", id))
		}
		return "", err
	}
