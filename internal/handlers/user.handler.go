...
func (u *UserHandler) Register(c *fiber.Ctx) (err error) {
	...
	return c.Status(fiber.StatusCreated).SendString(fmt.Sprintf("Success sign in, id: %d. Check email for activation", user.IdUser))
}
...
func (u *UserHandler) Activation(c *fiber.Ctx) (err error) {
	...
	user, err := u.repository.GetByToken(ctx, u.db, query.Token)
	if err != nil {
		return err
	}
